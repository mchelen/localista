...
/** FIPS state code → USPS abbreviation and name (states + DC + territories). */
export const STATES_BY_FIPS: Record<string, StateInfo> = {
...
export function stateFromFips(fips: string | undefined): StateInfo | undefined {
  if (!fips) return undefined
  return STATES_BY_FIPS[fips.padStart(2, '0')]
}
